    ...
    override fun handleInternal(request: BotRequest, alice: AliceReactions): State {
        ...
        alice.say(
            """
                ...
                Можно выбрать готовую тренировку вокруг Кремля, или бегать в своем любимом месте (например, дорожка в парке или стадион).
                Что выберете?
            """.trimIndent()
        )

        if (Random.nextBoolean()) { // todo: if geo enabled, return route, if not - return point
            alice.link(
